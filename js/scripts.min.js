...
var first = $('.first-screen');
var second = $('.second-screen');
var back = $('.order__to-left');
var next = $('.hided');
var form = false;
var win = 'first';
console.log(status);

$('.first-screen input').on('keydown', function () {
    checkInputs();
    slideWin();
    Copy();
    if (form === true && win === 'first') {
        next.fadeIn();
    }
});
function checkInputs() {
    let j = 0;
    first.find('.styledInput input').map(function (currentValue, index, array) {
        if ($(index).val(!'')) j++;
        console.log(j);
    });
    (j === first.find('input').length) ? form = true : form = false;
}
function slideWin() {
...
function Copy() {
    first.find('input').map(function (currentValue, index, array) {
        var name = $(index).attr('name');
        var newForm = second.find('input[name="' + name + '"]');
        $(newForm).val($(index).val());
    })
}


$('.burger').click(function () {
    $('.first__topline').removeClass('wow').removeClass('bounceInDown').css({
        display: 'inherit'
    });
    $('nav').toggle(300);
    if ($('.burger').hasClass('checked')) {
        $('.burger').removeClass('checked');
    }
    else {
        $('.burger').addClass('checked');
        $('.first__topline').css({ display: 'flex' });
    }
});

// function submitForm() {
//     return Promise.resolve(() => document.getElementById('submitData').submit())
// }
